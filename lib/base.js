...
class Base {
  ...
  static fetch(query, opts = {}) {
    ...
    return knex(Util.table(this))
      ...
      .then((res) => new this(Util.toCamel(res), opts));
  }
  ...
  save(opts = {}) {
    ...
    _.transform(this._properties, (result, value, key) => {
      result[_.snakeCase(key)] = value;
    }, this._safeProperties);

    return knex(Util.table(this))[method](this._safeProperties, '*')
      .spread((res) => {
        // Save computed values
        _.transform(res, (result, value, key) => {
          result[key] = value;
        }, this._properties);
   ...
   * @return {*} An updated copy of the model.
   */
  update(properties, opts = {}) {
    const knex = Util.assertKnex(opts.trx || opts.knex || this._trx || Config.knex);

    // Save computed values
    _.transform(properties, (result, value, key) => {
      result[key] = value;
    }, this._properties);
